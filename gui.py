...
def configure_entry_background(
    entry,
    value,
    limit_down,
    limit_up,
...
def get_current_price_work(label, symbol="BTCUSDT"):
    while True:
        label.config(text=f"{symbol} CURRENT PRICE: " + get_current_price(symbol))
        time.sleep(2)


def update_func(entries, vars, update_data_minutes_invertal, sleep_time=30, symbol="BTCUSDT"):
    while True:
        try:
            macdhist, cci, money_flow, rsi, slowd, bool_bands = get_data(
                minutes_interval=update_data_minutes_invertal, symbol=symbol
            )
...
def tab_maker(
    notebook, update_interval, thread_sleep_time=30, lbl_title="", tab_title="", symbol="BTCUSDT"
):
    frame = ttk.Frame(notebook)
    thread = threading.Thread(
        target=update_func,
        args=[*tab_populator(tab=frame, title=lbl_title, symbol=symbol), update_interval],
        kwargs={"symbol": symbol, "sleep_time": thread_sleep_time},
    )
...
def tab_populator(tab, title="", symbol="BTCUSDT"):
    if title:
    ...
    open_in_web_button = Button(
        tab,
        text=f"Open {symbol} in Binance",
        command=lambda: webbrowser.open(f"https://www.binance.com/en/futuresng/{symbol}", new=2),
    )
    ...
    with open("tabs.json", "r") as f:
        ...
        for tab in tabs:
            tab_maker(
                tabControl,
                tab["interval"],
                thread_sleep_time=tab["thread_sleep_time"],
                lbl_title=tab["lbl_title"].format(symbol),
                tab_title=tab["tab_title"],
                symbol=symbol,
            )

    current_price_thread = threading.Thread(
        target=get_current_price_work, args=[price_title, symbol]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--names-list",
        type=str,
        help="Symbol of the exchange that program should use, defaults to BTCUSDT",
        default=["BTCUSDT"],
        nargs="+",
    )
